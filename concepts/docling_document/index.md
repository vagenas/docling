...
The definition of the Pydantic types is implemented in the module docling\_core.types.doc, more details in source code definitions.
...
DoclingDocument converted from tests/data/word\_sample.docx and we present some side-by-side comparisons,
...
The first category is the content items, which are stored in these fields:
...
- key\_value\_items: All key-value items.
...
The second category is content structure, which is encapsualted in:
...
The reading order of the document is encapsulated through the body tree and the order of children in each item
in the tree.
